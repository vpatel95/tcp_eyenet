...
class tcp_transport {
    ...
    public:
        tcp_transport(T cong, sender_args args);
...
template <class T>
tcp_transport<T>::tcp_transport(T cong, sender_args args) :
    cong(cong),
    type(SENDER),
    sk(),
    dip(args.dst_ip),
...
#define _switch_sz (byte_switcher ? npkts * PAYLOAD : curr_ts)
    while (_switch_sz < fsz) {
        ...
        if ((curr_ts - last_s_ts >= cong.intersend * bn_train) ||
        ...
        if (cong.cwnd > 0)
            timeout = min(1000.0, last_s_ts + cong.intersend * bn_train - curr_ts);

        sockaddr_in addr;
        if(0 == sk.recv_data(buf, PKT_SZ, timeout, addr)) {
            curr_ts = get_curr_ts(start_ts);
            if (curr_ts > last_s_ts + cong.rto)
                cong.on_to();
            continue;
        }
